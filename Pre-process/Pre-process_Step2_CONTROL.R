...
SO <- merge(nk1, y = c(nk2, nk3), add.cell.ids = c("nk1", "nk2", "nk3"), project = "Diet K DCT")
...
SO$Rep <- factor(x = SO$Rep, levels = c("NK1", "NK2", "NK3")) 
...
SO@meta.data <- SO@meta.data %>% mutate(class = dplyr::case_when(
  ...
  seurat_clusters == 2 ~ "PT-S1",
  seurat_clusters == 3 ~ "TAL",
  seurat_clusters == 4 ~ "PC",
  seurat_clusters == 5 ~ "IC-B",
  seurat_clusters == 6 ~ "PT-S2",
  seurat_clusters == 7 ~ "PT-S1",
  seurat_clusters == 8 ~ "PT-S1",
  seurat_clusters == 9 ~ "EC",
  seurat_clusters == 10 ~ "Damaged nuclei",
  seurat_clusters == 11 ~ "Peri",
  seurat_clusters == 12 ~ "IC-A",
  seurat_clusters == 13 ~ "X",
...
SO.DCT <- subset(SO, idents = c("DCT1", "DCT2"))
...
## Adding Annotation Metadata to DCT Clusters (all cells as DCT), which will be used for pseudotime analysis 
SO.DCT@meta.data <- SO.DCT@meta.data %>% 
...
## This creates another metadata, 'analysis' with cell type and treatment
...
saveRDS(SO.DCT, "CONTROL_DCT.rds")
